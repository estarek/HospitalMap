...
class xml_parser(object):
    ...
    def getTitles(self,xml):
        ...
        node = doc.documentElement
        books = doc.getElementsByTagName("link")
        walls =[]
        for b in books:
            print(b.get_attributes())
            if "Wall" in b.get_tagName():
                walls.append(b)
        titles = []
        for w in walls:
            titleObj = w.getElementsByTagName("geometry")
...
if __name__ == "__main__":
    appt = xml_parser(r"model.sdf")
    appt.getTitles("model.sdf")
